...
public class GuiSimplyCrafter extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        for (int y = 0; y < 3; ++y)
            for (int x = 0; x < 3; ++x)
                if (!tileEntity.inventory.getInternalItemStackHandler().getStackInSlot(x + y * 3).isEmpty())
                    drawSingleStackCount(30 + x * 18, 17 + y * 18);

        if (tileEntity.recipe != null && tileEntity.inventory.getInternalItemStackHandler().getStackInSlot(9).getCount() == 1)
            drawSingleStackCount(124, 35);

        this.renderHoveredToolTip(mouseX-guiLeft,mouseY-guiTop);
    }

    private void drawSingleStackCount(int x, int y) {
        String s = "1";
